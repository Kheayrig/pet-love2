<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
    <title>Ardent Crew Pet Love service Page 19.04.22</title>
    <link rel="stylesheet" href="assets/bootstrap/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Libre+Franklin:400,700&amp;display=swap">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/baguettebox.js/1.11.1/baguetteBox.min.css">
    <link rel="stylesheet" href="assets/css/ad.css">
</head>

<body class="main-body">
    <nav class="navbar navbar-light navbar-expand-md" style="height: 85px;border-bottom: 1px solid #EDE5DE;">
    <div class="container-fluid">
        <div class="banner">
            <a href="./index.html" class="banner-link">
                <svg width="151" height="42" viewBox="0 0 151 42" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><rect width="151" height="42" fill="url(#pattern0)"/><defs><pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1"><use xlink:href="#image0_2_85" transform="translate(0 -0.00591674) scale(0.00299401 0.0107642)"/></pattern><image id="image0_2_85" width="334" height="94" xlink:href="data:image/png;base64,"/></defs></svg>
            </a>
        </div>
        <div class="collapse navbar-collapse d-xl-flex d-xxl-flex justify-content-xl-center align-items-xxl-end" id="navcol-1">
            <div class="select-div" onchange="">
                <select class="city-select">
                    <option selected value="1">Ярославль</option>
                    <option value="2">Москва</option>
                    <option value="3">Вологда</option>
                </select>
            </div>
            <ul class="navbar-nav text-start d-xxl-flex ms-auto align-items-xxl-center">
                <li class="nav-item"><a class="nav-link active fw-normal" href="#" style="color: #3D2127;transform: perspective(0px);padding-right: 0px;margin-right: 72px;margin-top: 10p;">Раздел<br>для ситтеров</a></li>
                <li class="nav-item fs-5 fw-normal d-lg-flex d-xl-flex align-items-lg-center align-items-xl-center" style="color: #3D2127;transform: perspective(0px);padding-right: 0px;margin-right: 72px;margin-top: 10p;"><a class="nav-link fs-6 fw-normal" href="#" style="color: #3D2127;transform: perspective(0px);padding: 9px 72px 8px 7px;padding-right: 0px;margin-top: 10p;">Создать заказ</a></li>
                <li class="nav-item d-lg-flex d-xl-flex align-items-lg-center align-items-xl-center"><a class="nav-link" href="#" style="color: #3D2127;margin-right: 119px;">Профиль</a></li>
            </ul>
        </div>
    </div>
</nav>
    <section>
        <div>
            <h1 class="overexposure-heading" style="text-align: left;color: #0099B3;"><strong>Передержка попугая</strong></h1>
        </div>
        <div>
            <div class="container">
                <div class="row">
                    <div class="col-md-8 col-xl-9" id="workers-left_column">
                        <div class="ad-profile">
                            <div class="d-flex flex-row justify-content-around ad-header">
                                <div class="profile-pic"></div>
                                <div class="profile-info">
                                    <p style="margin-bottom: 0;margin-top: 10px;">Вита</p>
                                    <p>Шевченко</p>
                                    <p style="margin-bottom: 0;margin-top: 55px;">Рейтинг 5.0</p>
                                    <p style="color: #118123;">Подтвержденный пользователь</p>
                                </div>
                                <div class="text-center profile-links">
                                    <div class="d-xxl-flex links-item" style="/*color: #ff0000;*/"><a class="text-center d-xl-flex d-xxl-flex justify-content-xl-center align-items-xl-center justify-content-xxl-center align-items-xxl-center link" id="link-1" href="#" style="/*height: 30px;*/">Написать сообщение</a></div>
                                    <div class="d-xxl-flex links-item" style="/*color: #ff0000;*/"><a class="text-center d-xl-flex d-xxl-flex justify-content-xl-center align-items-xl-center justify-content-xxl-center align-items-xxl-center link" id="link-2" href="#" style="/*height: 30px;*/">Аудиозвонок</a></div>
                                    <div class="d-xxl-flex links-item" style="/*color: #ff0000;*/"><a class="text-center d-xl-flex d-xxl-flex justify-content-xl-center align-items-xl-center justify-content-xxl-center align-items-xxl-center link" id="link-3" href="#" style="/*height: 30px;*/">Видеозвонок</a></div>
                                </div>
                            </div>
                            <div class="d-xxl-flex justify-content-xxl-center ad-desc">
                                <div class="border-primary d-xl-flex flex-column align-items-xl-center desc-cont">
                                    <div class="desc-heading">
                                        <h2 class="text-center" style="font-size: 24px;"><strong>Посижу с Вашим попугайчиком</strong></h2>
                                    </div>
                                    <div class="d-xxl-flex justify-content-xxl-center desc-par">
                                        <p class="d-xl-flex justify-content-xl-center align-items-xl-center" id="ad-paragraph" style="font-size: 15px;">Забота о любимом питомце на время Вашего отсутствия.<br>Передержка, в основном, попугайчиков, но возможен уход и за другими животными. Всё индивидуально, спрашивайте, не стесняйтесь.<br><br>Опыт ухода за мелкими животными - 5 лет. Каждый день могу отправлять фото и видео питомца) Имеется "игровая зона" для птичек, на которой они будут располагаться вместе с клеткой, игрушками и тд.<br>Свободу своим птичкам никогда не ограничивала, в клетке животное сидеть не будет.<br></p>
                                    </div>
                                </div>
                            </div>
                            <div class="d-xxl-flex justify-content-xxl-end ad-price">
                                <p class="d-xl-flex justify-content-xl-end" style="padding-right: 103px;">От 100 Р. за сутки</p>
                            </div>
                            <div class="ad-gallery">
                                <section class="photo-gallery py-4 py-xl-5">
                                    <div class="container flex-nowrap">
                                        <div class="row gx-2 gy-2 row-cols-1 row-cols-md-2 row-cols-xl-3 d-xl-flex d-xxl-flex justify-content-between justify-content-xl-center justify-content-xxl-center align-items-xxl-center photos" data-bss-baguettebox="">
                                            <div class="col item gallery-column" id="gallery-item-1"><a href="https://cdn.bootstrapstudio.io/placeholders/1400x800.png"></a></div>
                                            <div class="col item gallery-column" id="gallery-item-2"><a href="https://cdn.bootstrapstudio.io/placeholders/1400x800.png"></a></div>
                                            <div class="col item gallery-column" id="gallery-item-3"><a href="https://cdn.bootstrapstudio.io/placeholders/1400x800.png"></a></div>
                                            <div class="col item gallery-column" id="gallery-item-4"><a href="https://cdn.bootstrapstudio.io/placeholders/1400x800.png"></a></div>
                                        </div>
                                    </div>
                                </section>
                            </div>
                            <div class="comment-section">
                                <div class="comment-heading">
                                    <h3><strong>Отзывы</strong></h3>
                                </div>
                                <div class="d-xl-flex d-xxl-flex justify-content-xl-start comment-item">
                                    <div class="comment-image"></div>
                                    <div class="d-xxl-flex flex-column justify-content-xxl-start comment-cont">
                                        <p class="comment-text comment-name" style="color: #0099B3;">Григорий</p>
                                        <p class="comment-text">Прекрасная девушка! Каждый вечер показывала нашего попугая по видео связи. Советую!<br></p>
                                    </div>
                                </div>
                                <div class="d-xl-flex d-xxl-flex justify-content-xl-start comment-item">
                                    <div class="comment-image"></div>
                                    <div class="d-xxl-flex flex-column justify-content-xxl-start comment-cont">
                                        <p class="comment-text comment-name" style="color: #0099B3;">Григорий</p>
                                        <p class="comment-text">Прекрасная девушка! Каждый вечер показывала нашего попугая по видео связи. Советую!<br></p>
                                    </div>
                                </div>
                                <div class="d-xl-flex d-xxl-flex justify-content-xl-start comment-item">
                                    <div class="comment-image"></div>
                                    <div class="d-xxl-flex flex-column justify-content-xxl-start comment-cont">
                                        <p class="comment-text comment-name" style="color: #0099B3;">Григорий</p>
                                        <p class="comment-text">Прекрасная девушка! Каждый вечер показывала нашего попугая по видео связи. Советую!<br></p>
                                    </div>
                                </div>
                                <div class="d-xl-flex d-xxl-flex justify-content-xl-start comment-item">
                                    <div class="comment-image"></div>
                                    <div class="d-xxl-flex flex-column justify-content-xxl-start comment-cont">
                                        <p class="comment-text comment-name" style="color: #0099B3;">Григорий</p>
                                        <p class="comment-text">Прекрасная девушка! Каждый вечер показывала нашего попугая по видео связи. Советую!<br></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4 col-xl-3" id="workers-right_column">
                        <div class="d-xl-flex justify-content-xl-center align-items-xl-center">
                            <h1 class="fav-heading">Другие ситтеры</h1>
                        </div>
                        <ul class="list-group">
                            <li class="list-group-item d-xl-flex justify-content-between fav-list_item"><a class="d-xl-flex justify-content-xl-center" href="#">
                                    <div id="fav-1" class="fav-img"></div>
                                    <div class="d-xl-flex fav-desc">
                                        <p class="text-center" style="font-size: 16px;height: 100%;">Выгул собак и кошек<br>500м. от вас</p>
                                    </div>
                                </a></li>
                            <li class="list-group-item d-xl-flex justify-content-between fav-list_item"><a class="d-xl-flex justify-content-xl-center" href="#">
                                    <div id="fav-2" class="fav-img"></div>
                                    <div class="d-xl-flex fav-desc">
                                        <p class="text-center" style="font-size: 16px;height: 100%;">Выгул собак и кошек<br>500м. от вас</p>
                                    </div>
                                </a></li>
                            <li class="list-group-item d-xl-flex justify-content-between fav-list_item"><a class="d-xl-flex justify-content-xl-center" href="#">
                                    <div id="fav-3" class="fav-img"></div>
                                    <div class="d-xl-flex fav-desc">
                                        <p class="text-center" style="font-size: 16px;height: 100%;">Выгул собак и кошек<br>500м. от вас</p>
                                    </div>
                                </a></li>
                            <li class="list-group-item d-xl-flex justify-content-between fav-list_item"><a class="d-xl-flex justify-content-xl-center" href="#">
                                    <div id="fav-4" class="fav-img"></div>
                                    <div class="d-xl-flex fav-desc">
                                        <p class="text-center" style="font-size: 16px;height: 100%;">Выгул собак и кошек<br>500м. от вас</p>
                                    </div>
                                </a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <script src="assets/bootstrap/js/bootstrap.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/baguettebox.js/1.11.1/baguetteBox.min.js"></script>
    <script src="assets/js/Lightbox-Gallery.js"></script>
</body>

</html>